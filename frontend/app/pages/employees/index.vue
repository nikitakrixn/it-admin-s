<template>
  <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="mt-1 text-gray-600">Управление персоналом организации</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          ...
          <NuxtLink to="/employees/departments" class="btn btn-secondary">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            ...
            <div class="h-12 w-12 rounded-xl bg-blue-100 flex items-center justify-center">
              <Icon name="ri:user-line" class="text-2xl text-blue-600" />
            </div>
          </div>
        </div>
      </div>

      <div class="card cursor-pointer hover:shadow-md transition-shadow" @click="filters.status = 'active'; loadEmployees()">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
      ...
      <div class="card cursor-pointer hover:shadow-md transition-shadow" @click="filters.status = 'inactive'; loadEmployees()">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <td colspan="7" class="px-6 py-16 text-center">
                <div class="flex flex-col items-center">
                  <div class="h-20 w-20 rounded-full bg-gray-100 flex items-center justify-center mb-4">
                    <Icon name="ri:user-search-line" class="text-4xl text-gray-400" />
                  </div>
            ...
            <tr v-for="employee in employees" :key="employee.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-3">
                  <div class="h-10 w-10 rounded-full gradient-primary flex items-center justify-center text-white font-semibold text-sm shadow-sm">
                    {{ employee.first_name.charAt(0) }}{{ employee.last_name.charAt(0) }}
                  </div>
                  <div>
                    <div class="text-sm font-medium text-gray-900">{{ employee.full_name }}</div>
                    <div v-if="employee.ad_username" class="text-xs text-gray-500 flex items-center gap-1">
                      <Icon name="ri:windows-fill" class="text-xs" />
                      {{ employee.ad_username }}
                    </div>
                  </div>
                </div>
              </td>
              <td class="px-4 py-4 text-sm text-gray-900">{{ employee.position_name || '-' }}</td>
              <td class="px-4 py-4 text-sm text-gray-900">{{ employee.department_name || '-' }}</td>
              <td class="px-4 py-4">
                <div v-if="employee.email" class="text-sm text-gray-900 flex items-center gap-1 mb-1">
                  <Icon name="ri:mail-line" class="text-gray-400" />
                  <a :href="`mailto:${employee.email}`" class="text-primary-600 hover:underline">
                    {{ employee.email }}
                  </a>
                </div>
                <div v-if="employee.phone" class="text-xs text-gray-500 flex items-center gap-1">
                  <Icon name="ri:phone-line" class="text-gray-400" />
                  {{ employee.phone }}
                </div>
                <span v-if="!employee.email && !employee.phone" class="text-sm text-gray-400">-</span>
              </td>
              <td class="px-4 py-4">
                <span
                  :class="{
