    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Компьютеры</h1>
            <p class="mt-1 text-gray-600">Управление компьютерным парком</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <button @click="toggleView" class="btn btn-secondary">
            <Icon :name="viewMode === 'grid' ? 'ri:list-check' : 'ri:grid-line'" class="mr-2" />
    ...
    <div class="card">
      <div class="p-4">
        <div class="grid grid-cols-5 gap-4">
          <div class="cursor-pointer hover:bg-gray-50 rounded-lg p-3 transition-colors" @click="filterByStatus('active')">
          ...
          <div class="cursor-pointer hover:bg-gray-50 rounded-lg p-3 transition-colors" @click="filterByStatus('repair')">
            <div class="flex items-center gap-3">
          ...
          <div class="cursor-pointer hover:bg-gray-50 rounded-lg p-3 transition-colors" @click="filterByStatus('storage')">
            <div class="flex items-center gap-3">
          ...
          <div class="cursor-pointer hover:bg-gray-50 rounded-lg p-3 transition-colors" @click="filterByStatus('inactive')">
            <div class="flex items-center gap-3">
          ...
          <div class="cursor-pointer hover:bg-gray-50 rounded-lg p-3 transition-colors">
            <div class="flex items-center gap-3">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Компьютер</th>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">ОС</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Характеристики</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Статус</th>
              <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Действия</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="computer in filteredComputers" :key="computer.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-4 py-4">
                <div class="flex items-center gap-3">
                  <div :class="['h-10 w-10 rounded-lg flex items-center justify-center', getTypeColor(computer.computer_type)]">
                    <Icon :name="getTypeIcon(computer.computer_type)" class="text-xl text-white" />
                  </div>
                  <div>
                    <p class="text-sm font-medium text-gray-900">{{ computer.hostname }}</p>
                    <p class="text-xs text-gray-500">{{ computer.inventory_number }}</p>
                  </div>
                </div>
              </td>
              <td class="px-4 py-4">
              ...
              <td class="px-4 py-4">
                <div v-if="computer.employee" class="flex items-center gap-2">
                  <Icon name="ri:user-line" class="text-gray-400" />
                  <span class="text-sm text-gray-900">{{ computer.employee }}</span>
                </div>
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    <p class="text-sm text-gray-900">{{ computer.os }}</p>
                    <p class="text-xs text-gray-500">{{ computer.os_version }}</p>
                  </div>
                </div>
              </td>
              <td class="px-4 py-4">
                <div class="text-xs text-gray-600 space-y-1">
                  <div class="flex items-center gap-1">
                    <Icon name="ri:cpu-line" class="text-gray-400" />
                    <span>{{ computer.cpu }}</span>
                  </div>
                  <div class="flex items-center gap-1">
                    <Icon name="ri:database-2-line" class="text-gray-400" />
                    <span>{{ computer.ram }} ГБ RAM</span>
                  </div>
                  <div class="flex items-center gap-1">
                    <Icon name="ri:hard-drive-line" class="text-gray-400" />
              ...
              <td class="px-4 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <NuxtLink :to="`/computers/${computer.id}`" class="p-2 rounded-lg hover:bg-gray-100 transition-colors" title="Просмотр">
                    <Icon name="ri:eye-line" class="text-gray-600" />
                  </NuxtLink>
                  <NuxtLink :to="`/computers/${computer.id}/edit`" class="p-2 rounded-lg hover:bg-gray-100 transition-colors" title="Редактировать">
                    <Icon name="ri:edit-line" class="text-gray-600" />
                  </NuxtLink>
                </div>
              </td>
            </tr>
        ...
        <div v-if="filteredComputers.length === 0" class="p-16 text-center">
          <div class="h-20 w-20 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4">
            <Icon name="ri:computer-line" class="text-4xl text-gray-400" />
          </div>
          <h3 class="text-lg font-semibold text-gray-900 mb-2">Компьютеры не найдены</h3>
          <p class="text-sm text-gray-500 mb-4">Попробуйте изменить параметры поиска</p>
        </div>
      </div>
    </div>
      ...
      >
        <div class="p-5">
          <div class="flex items-start justify-between mb-4">
            <div :class="['h-12 w-12 rounded-xl flex items-center justify-center', getTypeColor(computer.computer_type)]">
              <Icon :name="getTypeIcon(computer.computer_type)" class="text-2xl text-white" />
            </div>
            <span :class="['badge', getStatusColor(computer.status)]">
              {{ getStatusLabel(computer.status) }}
            </span>
          ...
          <p class="text-sm text-gray-500 mb-3">{{ computer.inventory_number }}</p>

          <div class="space-y-2 mb-4">
            <div class="flex items-center gap-2 text-sm text-gray-600">
              <Icon :name="getOSIcon(computer.os)" class="text-gray-400" />
              <span>{{ computer.os }} {{ computer.os_version }}</span>
            </div>
            <div v-if="computer.employee" class="flex items-center gap-2 text-sm text-gray-600">
              <Icon name="ri:user-line" class="text-gray-400" />
          ...
          <div class="pt-3 border-t border-gray-200 space-y-1">
            ...
            <div class="flex items-center justify-between text-xs text-gray-600">
              ...
              <span class="font-medium">{{ computer.ram }} ГБ</span>
            </div>
            <div class="flex items-center justify-between text-xs text-gray-600">
              <span class="flex items-center gap-1">
              ...
              <span class="font-medium">{{ computer.storage }} ГБ</span>
            </div>
          </div>
        </div>
      </NuxtLink>

      <div v-if="filteredComputers.length === 0" class="col-span-full p-16 text-center">
        <div class="h-20 w-20 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4">
          <Icon name="ri:computer-line" class="text-4xl text-gray-400" />
        </div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Компьютеры не найдены</h3>
...
const filteredComputers = computed(() => {
  ...
  if (selectedType.value) {
    filtered = filtered.filter(c => c.computer_type === selectedType.value)
  }
