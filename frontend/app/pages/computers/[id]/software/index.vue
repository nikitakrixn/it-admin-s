  ...
  <div class="space-y-6">
    <div class="mb-8">
      <div class="flex items-center gap-4 mb-6">
        <NuxtLink :to="`/computers/${route.params.id}`" class="p-2 rounded-lg hover:bg-gray-100 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-purple-600">{{ softwareList.length }}</p>
            </div>
            <div class="h-12 w-12 rounded-xl bg-purple-100 flex items-center justify-center">
              <Icon name="ri:apps-line" class="text-2xl text-purple-600" />
            </div>
          </div>
        </div>
      </div>
      <div class="card">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 mb-1">Лицензионное</p>
              <p class="text-2xl font-bold text-green-600">{{ licensedCount }}</p>
            </div>
            <div class="h-12 w-12 rounded-xl bg-green-100 flex items-center justify-center">
              <Icon name="ri:shield-check-line" class="text-2xl text-green-600" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-amber-600">{{ expiringCount }}</p>
            </div>
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm font-medium text-gray-900">{{ lastScan }}</p>
            </div>
            <div class="h-12 w-12 rounded-xl bg-blue-100 flex items-center justify-center">
              <Icon name="ri:refresh-line" class="text-2xl text-blue-600" />
            </div>
          </div>
        </div>
      </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Версия</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="sw in filteredSoftware" :key="sw.id" class="hover:bg-gray-50">
              <td class="px-4 py-4">
                <div class="flex items-center gap-3">
                  <Icon name="ri:apps-2-line" class="text-gray-400" />
                  <div>
                    <p class="text-sm font-medium text-gray-900">{{ sw.name }}</p>
                    <p class="text-xs text-gray-500">{{ sw.manufacturer }}</p>
                  </div>
                </div>
              </td>
              <td class="px-4 py-4">
                <p class="text-sm text-gray-900">{{ sw.version }}</p>
              </td>
              <td class="px-4 py-4">
                <span v-if="sw.license_type" :class="['badge', getLicenseColor(sw.license_type)]">
                  {{ getLicenseLabel(sw.license_type) }}
                </span>
                <span v-else class="text-sm text-gray-400">-</span>
              </td>
              <td class="px-4 py-4">
                <p class="text-sm text-gray-600">{{ sw.installation_date }}</p>
              </td>
              <td class="px-4 py-4 text-right">
                <button @click="removeSoftware(sw)" class="p-2 rounded-lg hover:bg-red-50 transition-colors" title="Удалить">
                  <Icon name="ri:delete-bin-line" class="text-red-600" />
                </button>
              </td>
            </tr>
          </tbody>
...
const softwareList = ref([
  { id: 1, name: 'Microsoft Office 365', manufacturer: 'Microsoft', version: '16.0.16827', license_type: 'subscription', installation_date: '15.01.2024', category: 'office' },
  { id: 2, name: 'Google Chrome', manufacturer: 'Google', version: '120.0.6099', license_type: 'free', installation_date: '15.01.2024', category: 'browser' },
  { id: 3, name: 'Visual Studio Code', manufacturer: 'Microsoft', version: '1.85.1', license_type: 'free', installation_date: '20.03.2024', category: 'development' }
])
...
const licensedCount = computed(() => softwareList.value.filter(s => s.license_type !== 'free').length)
