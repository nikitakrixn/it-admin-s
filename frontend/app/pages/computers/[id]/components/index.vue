        ...
        <div class="flex-1 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div class="h-16 w-16 rounded-2xl bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center shadow-lg">
              <Icon name="ri:cpu-line" class="text-3xl text-white" />
            </div>
            <div>
              <h1 class="text-3xl font-bold text-gray-900">Компоненты</h1>
              <p class="mt-1 text-gray-600">{{ computerName }}</p>
            </div>
          </div>

          <NuxtLink :to="`/computers/${route.params.id}/components/add`" class="btn btn-primary">
        ...
        <div class="p-5">
          <div class="flex items-start justify-between mb-4">
            <div class="flex items-center gap-3">
              <div :class="['h-12 w-12 rounded-xl flex items-center justify-center', getComponentColor(component.type)]">
                <Icon :name="getComponentIcon(component.type)" class="text-2xl text-white" />
              </div>
              <div>
                <p class="text-sm font-semibold text-gray-900 group-hover:text-primary-600 transition-colors">
                  {{ getComponentTypeLabel(component.type) }}
                </p>
                <p class="text-xs text-gray-500">{{ component.name }}</p>
              </div>
            </div>
          ...
          <div class="space-y-2">
            <div v-for="(value, key) in component.specs" :key="key" class="flex items-center justify-between text-sm">
              <span class="text-gray-600">{{ key }}</span>
              <span class="font-medium text-gray-900">{{ value }}</span>
            </div>
          </div>

          <div v-if="component.notes" class="mt-3 pt-3 border-t border-gray-200">
            <p class="text-xs text-gray-600">{{ component.notes }}</p>
          </div>
        </div>
      </div>

      <div v-if="filteredComponents.length === 0" class="col-span-full p-16 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Компоненты не найдены</h3>
        <p class="text-sm text-gray-500 mb-4">Добавьте первый компонент</p>
        <NuxtLink :to="`/computers/${route.params.id}/components/add`" class="btn btn-primary">
          <Icon name="ri:add-line" class="mr-2" />
          Добавить компонент
        </NuxtLink>
...
const editComponent = (component: any) => {
  router.push(`/computers/${route.params.id}/components/${component.id}/edit`)
}
