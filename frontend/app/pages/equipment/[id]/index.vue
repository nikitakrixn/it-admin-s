  ...
  <div class="space-y-6">
    <div class="mb-8">
      <div class="flex items-center gap-4 mb-6">
        <NuxtLink to="/equipment" class="p-2 rounded-lg hover:bg-gray-100 transition-colors">
          <Icon name="ri:arrow-left-line" class="text-xl" />
        </NuxtLink>
        <div class="flex-1 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div :class="['h-16 w-16 rounded-2xl flex items-center justify-center shadow-lg', getTypeGradient(equipment.type)]">
              <Icon :name="getTypeIcon(equipment.type)" class="text-3xl text-white" />
            </div>
            <div>
              <div class="flex items-center gap-3">
                <h1 class="text-3xl font-bold text-gray-900">{{ equipment.name }}</h1>
                <span :class="['badge', getStatusColor(equipment.status)]">
                  {{ getStatusLabel(equipment.status) }}
                </span>
              </div>
              <p class="mt-1 text-gray-600">{{ equipment.inventory_number }}</p>
            </div>
          </div>

          <div class="flex items-center gap-2">
            <NuxtLink :to="`/equipment/${equipment.id}/edit`" class="btn btn-secondary">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-6">
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                ...
                <p class="text-sm font-medium text-gray-900">{{ equipment.manufacturer }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="text-sm font-medium text-gray-900 font-mono">{{ equipment.serial_number }}</p>
              </div>
              <div>
              ...
              <div v-if="equipment.ip_address">
                <p class="text-xs text-gray-600 mb-1">IP-адрес</p>
                <p class="text-sm font-medium text-gray-900 font-mono">{{ equipment.ip_address }}</p>
              </div>
            </div>
            <div v-if="equipment.notes" class="mt-6 pt-6 border-t border-gray-200">
        ...
        <div v-if="equipment.employee" class="card">
          ...
          <div class="p-6">
            <NuxtLink :to="`/employees/${equipment.employee.id}`" class="flex items-center gap-4 p-4 rounded-lg bg-blue-50 border border-blue-100 hover:bg-blue-100 transition-colors">
              <div class="h-12 w-12 rounded-full bg-blue-500 flex items-center justify-center">
                <Icon name="ri:user-line" class="text-xl text-white" />
              </div>
              <div>
                <p class="text-sm font-medium text-gray-900">{{ equipment.employee.name }}</p>
                <p class="text-xs text-gray-600">{{ equipment.employee.position }}</p>
              </div>
            </NuxtLink>
          </div>
        </div>
        ...
        <div class="card">
          <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
            <h2 class="text-lg font-semibold text-gray-900">История обслуживания</h2>
          </div>
          <div class="p-6">
            <div class="space-y-4">
              <div v-for="(event, index) in history" :key="event.id" class="flex gap-4">
                <div class="flex flex-col items-center">
                  <div :class="['h-8 w-8 rounded-full flex items-center justify-center', getEventColor(event.type)]">
      ...
      <div class="space-y-6">
        <div class="card">
          ...
          <div class="p-6">
            <select v-model="equipment.status" @change="updateStatus" class="input-field">
        ...
        <div class="card">
          <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
            <h3 class="text-sm font-semibold text-gray-900">Гарантия</h3>
          </div>
          <div class="p-6 space-y-4">
            <div>
              <p class="text-xs text-gray-600 mb-1">Дата покупки</p>
              <p class="text-sm font-medium text-gray-900">{{ equipment.purchase_date || '-' }}</p>
            </div>
            <div>
              <p class="text-xs text-gray-600 mb-1">Гарантия до</p>
              <div class="flex items-center gap-2">
                <p class="text-sm font-medium text-gray-900">{{ equipment.warranty_end_date || '-' }}</p>
                <span v-if="equipment.warranty_active" class="badge bg-green-100 text-green-800 text-xs">
                  Активна
                </span>
              </div>
            </div>
          ...
          <div class="p-6 space-y-3">
            <div class="flex items-center gap-2 text-sm">
              ...
              <div>
                ...
                <p class="text-gray-900">{{ equipment.created_at }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2 text-sm">
              ...
              <div>
                ...
                <p class="text-gray-900">{{ equipment.updated_at }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
              <div class="text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Удалить оборудование?</h3>
                ...
                <div class="flex gap-3">
                  ...
                  <button @click="deleteEquipment" class="flex-1 btn bg-red-600 text-white hover:bg-red-700">Удалить</button>
                </div>
              </div>
            </div>
          </div>
...
const history = ref([
  { id: 1, type: 'created', user: 'Администратор', action: 'добавил оборудование', details: null, created_at: '15.01.2024 10:00' },
  { id: 2, type: 'assigned', user: 'Администратор', action: 'назначил оборудование', details: 'Иванов И.И.', created_at: '15.01.2024 10:15' },
  { id: 3, type: 'maintenance', user: 'Техник', action: 'провел обслуживание', details: 'Замена картриджа', created_at: '20.06.2024 14:00' }
])
...
const updateStatus = () => {
  toast.success(`Статус изменен на "${getStatusLabel(equipment.value.status)}"`)
}
