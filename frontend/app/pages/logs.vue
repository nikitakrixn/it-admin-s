<template>
  <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="h-16 w-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center shadow-lg">
            <Icon name="ri:history-line" class="text-3xl text-white" />
          </div>
          <div>
            <h1 class="text-3xl font-bold text-gray-900">Журнал активности</h1>
            <p class="mt-1 text-gray-600">История действий в системе</p>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <div class="card cursor-pointer hover:shadow-md transition-shadow" @click="filterByAction('create')">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
            ...
            <div class="h-12 w-12 rounded-xl bg-green-100 flex items-center justify-center">
              <Icon name="ri:add-circle-line" class="text-2xl text-green-600" />
            </div>
          </div>
        </div>
      </div>

      <div class="card cursor-pointer hover:shadow-md transition-shadow" @click="filterByAction('update')">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
            ...
            <div class="h-12 w-12 rounded-xl bg-blue-100 flex items-center justify-center">
              <Icon name="ri:edit-line" class="text-2xl text-blue-600" />
            </div>
          </div>
        </div>
      </div>

      <div class="card cursor-pointer hover:shadow-md transition-shadow" @click="filterByAction('delete')">
        <div class="p-5">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 mb-1">Удалено</p>
              <p class="text-2xl font-bold text-red-600">{{ deleteCount }}</p>
            </div>
            <div class="h-12 w-12 rounded-xl bg-red-100 flex items-center justify-center">
              <Icon name="ri:delete-bin-line" class="text-2xl text-red-600" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="p-5">
          <div class="flex items-center justify-between">
            ...
            <div class="h-12 w-12 rounded-xl bg-purple-100 flex items-center justify-center">
              <Icon name="ri:file-list-line" class="text-2xl text-purple-600" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card">
      <div class="p-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
        <div class="flex items-center gap-4">
          ...
          <select v-model="filters.action_type" @change="loadLogs" class="input-field w-48">
            <option value="">Все действия</option>
            <option value="create">Создание</option>
            <option value="update">Изменение</option>
            <option value="delete">Удаление</option>
          </select>
          <select v-model="filters.entity_type" @change="loadLogs" class="input-field w-48">
      ...
      <div v-else class="divide-y divide-gray-200">
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-4">
                ...
                <Icon name="ri:arrow-right-s-line" class="text-gray-400 group-hover:text-primary-600 transition-colors" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="!logs.length" class="p-16 text-center">
          ...
          <p class="text-sm text-gray-500 mb-4">Попробуйте изменить параметры фильтров</p>
      ...
      <div v-if="data && data.total > 0" class="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-gray-50">
        <p class="text-sm text-gray-700">
          Показано <span class="font-medium">{{ Math.min((currentPage - 1) * perPage + 1, data.total) }}</span> - 
          <span class="font-medium">{{ Math.min(currentPage * perPage, data.total) }}</span> из 
...
const createCount = computed(() => logs.value.filter(l => l.action.toLowerCase() === 'create' || l.action.toLowerCase() === 'created').length)
const updateCount = computed(() => logs.value.filter(l => l.action.toLowerCase() === 'update' || l.action.toLowerCase() === 'updated').length)
const deleteCount = computed(() => logs.value.filter(l => l.action.toLowerCase() === 'delete' || l.action.toLowerCase() === 'deleted').length)
...
const exportLogs = () => {
  toast.success('Экспорт журнала начат. Файл будет загружен через несколько секунд.')
}
...
const getActionDescription = (log: any) => {
  ...
  const entity = entityLabels[log.entity_type.toLowerCase()] || log.entity_type
