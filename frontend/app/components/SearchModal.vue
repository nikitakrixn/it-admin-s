        ...
        <div class="relative min-h-screen flex items-start justify-center p-4 pt-20">
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
            <div class="p-4 border-b border-gray-200">
              <div class="relative">
                <Icon name="ri:search-line" class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-xl" />
                <input
                  ref="searchInput"
                  v-model="query"
                  type="text"
            ...
            <div v-if="query" class="max-h-96 overflow-y-auto scrollbar-thin">
              ...
              <div v-else-if="results.length === 0" class="p-8 text-center">
                <Icon name="ri:search-line" class="text-4xl text-gray-300 mx-auto mb-2" />
              ...
              <div v-else class="divide-y divide-gray-100">
                ...
                >
                  ...
                  >
                    <Icon :name="getTypeIcon(result.type)" class="text-lg text-white" />
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900">{{ result.title }}</p>
                    <p class="text-xs text-gray-500">{{ result.subtitle }}</p>
                  </div>
                  <span class="badge bg-gray-100 text-gray-600 text-xs">{{ getTypeLabel(result.type) }}</span>
                </NuxtLink>
              </div>
            </div>
